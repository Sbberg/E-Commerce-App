...
public interface ProductRepo extends JpaRepository<Product,Integer> {
    ...
    @Modifying
    @Query(value = "UPDATE products SET p_name=?1, p_price=?2, p_category=?3, p_inventory=?4, order_o_id=?5 WHERE p_id=?6", nativeQuery = true)
    int update(String productName, double price, String productCategory, int inventory, Order order, int productId);
